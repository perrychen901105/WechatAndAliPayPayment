...
- (NSString *)genPackage
{
    ...
    [params setObject:MCH_ID forKey:@"partner"];
    [params setObject:[WXUtil getIPAddress:YES] forKey:@"spbill_create_ip"];
    [params setObject:@"2" forKey:@"total_fee"];    // 1 =＝ ¥0.01
    ...
    [package appendString:@"key="];
    [package appendString:PARTNER_ID]; // 注意:不能hardcode在客户端,建议genPackage这个过程都由服务器端完成
...
{
    self.timeStamp = [self genTimeStamp];  // WEB SERVER
    self.nonceStr = [self genNonceStr]; // traceId 由开发者自定义，可用于订单的查询与跟踪，建议根据支付用户信息生成此id
    self.traceId = [self genTraceId];   // WEB SERVER
...
{
    NSString *getAccessTokenUrl = [NSString stringWithFormat:@"https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%@&secret=%@", APP_ID, APP_SECRET];

    __block NSString *strAccessToken = @"";
    NSURLSession *session = [NSURLSession sharedSession];
    [[session dataTaskWithURL:[NSURL URLWithString:getAccessTokenUrl]
            completionHandler:^(NSData *data,
                                NSURLResponse *response,
                                NSError *error) {
                // handle response
                if (!error) {
                    NSDictionary *dicts = [NSJSONSerialization JSONObjectWithData:data
                                                                         options:kNilOptions
                                                                           error:&error];
                    // 获取 access token
                    strAccessToken = dicts[AccessTokenKey];
                    NSString *getPrepayIdUrl = [NSString stringWithFormat:@"https://api.weixin.qq.com/pay/genprepay?access_token=%@", strAccessToken];
                    ...
                    // 获取商品参数
                    NSMutableData *postData = [self getProductArgs];//[NSJSONSerialization dataWithJSONObject:mapData options:0 error:&JSerror];
